...
def feedall(request):
    ...
    if request.method == "POST":
        sort = request.POST.get("sort")
        if sort != "":
            olist = Image.objects.filter(city_id=city).order_by(sort).reverse()
    else:
        # 실시간 (기본)
        olist = Image.objects.filter(city_id=city).order_by('image_date').reverse()

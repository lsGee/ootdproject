...
L.Control.zoomHome = L.Control.extend({
    ...
    options: {
        position: 'topright',
        zoomHomeText: `
            <svg width="22px" height="22px" viewBox="0 0 16 16" class="bi bi-arrows-fullscreen" fill="darkgrey" xmlns="http://www.w3.org/2000/svg">
    ...
    var msg = `
        <div><div>
            ...
            <div class="popup info">
                ...
                ${img.best_img}<br>
            <div class="btn-group">
                    <button type="button" class="button" onclick="location.href='${url[0]}'">더보기</button>
                    <button type="button" class="button" onclick="location.href='${url[1]}'">사진 올리기</button>
            </div>
            </div>
        </div></div>`;

        $("#map-cover, div.popup-area").css("display", "block");
        $("div.popup-area").attr("class", "popup-area active");
        $("#map-popup").html(msg);
}
...
function closePopup() {
    ...
    if(e.textContent == "Close")
        ...
        for(var i in popup_area) {
            ...
            // 다른 영역 클릭 시 팝업창 닫기
                ...
                // $("div.popup-area.active").attr("class", "popup-area");
                closed = true;
            }
        }
    }
    ...
    if(closed == true) {
        $("div.popup-area").removeClass("active");
        $("div.popup-area").attr("class", "popup-area active-c");
